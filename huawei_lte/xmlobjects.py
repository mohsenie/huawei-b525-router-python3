...
class XmlObject(object):
    ...
    def buildXmlRequest(self):
        return self.buildXML(root='request')

    def buildXmlResponse(self):
        return self.buildXML(root='response')

    def buildXmlError(self):
        return self.buildXML(root='error')
...
class TestFunctions(XmlObject):
    ...
    def getPropertyNames(self):
        return ['DeviceName', 'ProductFamily', 'HardwareVersion', 'SoftwareVersion', 'WebUIVersion', 'MacAddress1',
                'MacAddress2', 'Failed', 'Passed']
    ...
    def __init__(self, config):
        ...
        if isinstance(config, six.string_types):
            ...
            if protocol not in self.PROTOCOLS:
                raise ValueError(
                    'Invalid protocol specified for port forwarding (Virtual Server). Must be one of: [%s]' % ', '.join(
                        self.PROTOCOLS.keys))
            protocol = self.PROTOCOLS[protocol]
            if not utils.isIpValid(localIp):
                raise ValueError('Invalid ip address specified for port forwarding target server')
...
class LanSettings(XmlObject):
    ...
    def setDnsManual(self, config):
        ...
        if (secondaryDns != '' and not utils.isIpValid(secondaryDns)): raise ValueError(
            "Invalid Secondary DNS IP Address: %s" % secondaryDns)
        self.DnsStatus = 0
    ...
    def setLanAddress(self, config):
        ...
        if not utils.isIpValid(ipaddress):
            raise ValueError("Invalid LAN IP Address: %s" % ipaddress)
        if not utils.isIpValid(netmask):
            raise ValueError("Invalid LAN Netmask: %s" % netmask)
    ...
    def set(self, mode, config=False):
        ...
        if mode == self.MODE_DYNAMIC or mode == self.MODE_PPPOE_DYNAMIC or mode == self.MODE_AUTO:
            if 'primarydns' in config:
                ...
                if (dns != self.NO_DNS):
                    if (not utils.isIpValid(dns)): raise ValueError(
                        "Invalid IP Address for Dynamic Primary DNS: %s" % dns)
            ...
            if 'secondarydns' in config:
                ...
                if (dns != self.NO_DNS):
                    if (not utils.isIpValid(dns)): raise ValueError(
                        "Invalid IP Address for Dynamic Secondary DNS: %s" % dns)
            ...
            if 'dnsmanual' in config:
                ...
                if self.dynamicprimarydns == self.NO_DNS: raise ValueError(
        ...
        if mode == self.MODE_PPPOE or mode == self.MODE_PPPOE_DYNAMIC or mode == self.MODE_AUTO:
            if 'password' in config:
                ...
                if len(val) > 63: raise ValueError(
            ...
            if 'authmode' in config:
                ...
                if not mode in [self.AUTH_AUTO, self.AUTH_PAP, self.AUTH_CHAP]: raise ValueError(
